...
@Component({
  selector: 'lib-edge-handle',
  templateUrl: './edge-handle.component.html',
  styleUrls: ['./edge-handle.component.css']
})
export class EdgeHandleComponent implements OnInit {
  node_name: string;
  ...
  edgehandleMode$: Subject<any> = new Subject();
  ...
  edgehandleModeOnOff(mode:boolean) {
    this.edgehandleMode$.next(mode);
  }
}
